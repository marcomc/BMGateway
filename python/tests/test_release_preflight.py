...
def test_validate_release_version_state_accepts_current_repository() -> None:
    ...
    assert state.active_release_version == "0.4.1"
...
def test_cli_reports_current_shipped_release_state(capsys: pytest.CaptureFixture[str]) -> None:
    root = Path(__file__).resolve().parents[2]
    ...
    assert "active_release=0.4.1" in output
