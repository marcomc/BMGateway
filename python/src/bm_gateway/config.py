...
class BluetoothConfig:
    ...
    live_hard_timeout_seconds: int = 0
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "bluetooth": {
                ...
                "live_hard_timeout_seconds": self.bluetooth.live_hard_timeout_seconds,
            },
            "mqtt": {
...
def write_config(path: Path, config: AppConfig) -> None:
    payload = "\n".join(
        [
            ...
            f"live_hard_timeout_seconds = {config.bluetooth.live_hard_timeout_seconds}",
            "",
            "[mqtt]",
...
def load_config(path: Path) -> AppConfig:
    ...
    legacy_bm200_live_hard_timeout_seconds = int(
        bluetooth_table.get("bm200_live_hard_timeout_seconds", 0)
    )
    legacy_bm300_live_hard_timeout_seconds = int(
    ...
    bluetooth = BluetoothConfig(
        ...
        live_hard_timeout_seconds=int(
            bluetooth_table.get(
                "live_hard_timeout_seconds",
                max(
                    legacy_bm200_live_hard_timeout_seconds,
                    legacy_bm300_live_hard_timeout_seconds,
                ),
            )
        ),
    )
...
def validate_config(config: AppConfig) -> list[str]:
    ...
    if config.bluetooth.live_hard_timeout_seconds < 0:
        errors.append("bluetooth.live_hard_timeout_seconds must be zero or greater")
