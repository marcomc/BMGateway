...
| Lifecycle notification units | `bm-gateway-lifecycle.service` and `bm-gateway-boot-notification.service` | generated by `rpi-setup/scripts/install-service.sh` |
...
| `bm-gateway lifecycle boot` and `bm-gateway lifecycle shutdown` | Record system lifecycle notification intents; they do not control host power |
| `bm-gateway run` | Execute the runtime |
| `bm-gateway update report` | Queue a durable bootstrap update outcome |
